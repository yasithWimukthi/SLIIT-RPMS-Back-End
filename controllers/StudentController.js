...
export const getCompletedSubmissions = async (req, res, next) => {
  ...
  try {
    ...
    const submissions = await Submission.find(
      {
        submissions: {
          $elemMatch: { groupID: mongoose.Types.ObjectId(groupID) },
        },
      },
      {
        "submissions.$": true,
...
export const makeSubmission = async (req, res, next) => {
  ...
  try {
    const submission = await Submission.findByIdAndUpdate(
      mongoose.Types.ObjectId(submissionID),
      {
        $push: {
          submissions: {
            groupID,
...
export const getInCompletedSubmissions = async (req, res, next) => {
  ...
  try {
    const submissions = await Submission.find({
      submissions: {
        $not: { $elemMatch: { groupID: mongoose.Types.ObjectId(groupID) } },
      },
    });
    res.status(200).json({
      message: "Submissions retrieved successfully",
